...
struct matrix * generate_sphere(double cx, double cy, double cz,
                                double r, int step ) {
  struct matrix * ret_m = new_matrix(4, 0);
  ...
  for(i=1; i<=step; i++) {
    ...
    for(j=1; j<=step; j++) {
      ...
      nx = r*cos(theta) + cx;
      ny = r*sin(theta)*cos(phi) + cy;
      nz = r*sin(theta)*sin(phi) + cz;

      add_point(ret_m, nx, ny, nz);      
    }
  }
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  for(i=1; i<=step; i++) {
    ...
    for(j=1; j<=step; j++) {
      ...
      nx = cos(phi)*(r1*cos(theta) + r2) + cx;
      ny = r1*sin(theta) + cy;
      nz = sin(phi)*(r1*cos(theta) + r2) + cz;
