...
/*
Returns array containing components of cross product 
of vector A (ax, ay, az) and vector B (bx, by, bz)

i   j   k
ax  ay  az
bx  by  bz
*/
double *cross_prod(double ax, double ay, double az, double bx, double by, double bz) {
  double *retvec = malloc(3 * sizeof(double));
  retvec[0] = ay*bz - az*by;
  retvec[1] = az*bx - ax*bz;
  retvec[2] = ax*by - ay*bx;
