...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for (point=0; point < polygons->lastcol-1; point+=3) {
    ...
    if(DONT_OVERLAP) {
      ...
      resvec = cross_prod(ax, ay, az, bx, by, bz);
    }
    
    if(resvec != NULL && resvec[2] > 0) {
...
void add_torus( struct matrix * polygons, 
  ...
  int index, aindex, bindex, next_index, lat, longt;
  int latStop, longStop, latStart, longStart;
  latStart = 0;
  latStop = step;
  longStart = 0;
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      aindex = (index + 1) % (latStop * longStop);
      next_index = (index + longStop) % (latStop * longStop);
      bindex = (index + longStop + 1) % (latStop * longStop);
